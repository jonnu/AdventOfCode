        ...
        try (BufferedReader reader = resourceReader.read("y2023/puzzle10.txt")) {
            ...
            while (!queue.isEmpty()) {
                ...
                List<Point> points = point.cardinalNeighbours()
                        ...
                        .peek(p -> log.debug("I'm going from {} ({}) into {} ({}) [connections: {}; opposite: {}]", point, grid.get(point).getAlternative(), p.getValue(), grid.get(p.getValue()).getAlternative(), CONNECTIONS.get(grid.get(p.getValue())), p.getKey().opposite()))
